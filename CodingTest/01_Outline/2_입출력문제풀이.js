// 백준
    ...
    // 시간 제한 : (1초) 안에 종료되는 코드를 완성해야 한다.
    // 메모리 제한 : (128MB) 메모리가 넘지 않는 코드를 완성해야 한다.
    // 언어는 JavaScript 인 경우 Node.js 선택(설정에서 node.js 드래그해서 맨 위로 올리면 제출시 편하다.)
    // 백준(& Node.js)에서는 프로그래머스와 다르게 먼저 데이터를 읽어와야한다.
